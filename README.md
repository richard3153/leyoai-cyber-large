...
LeyoAI Cyber Model (Large) 是基于 Qwen2.5-7B-Instruct 微调的网络安全助手，提供最强的安全分析能力，适用于复杂场景。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-cyber-large",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-cyber-large")

# Inference
prompt = "分析这个 Advanced Persistent Threat (APT) 攻击的特征和防御策略"
...
LeyoAI Cyber Model (Large) is a cybersecurity assistant fine-tuned from Qwen2.5-7B-Instruct, providing the strongest security analysis capabilities for complex scenarios.
...
# Inference
prompt = "Analyze the characteristics and defense strategies of this Advanced Persistent Threat (APT) attack"
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
